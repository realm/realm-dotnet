...
namespace RealmNet.Interop
{
    internal class GroupHandle:RealmHandleOptionalUnbind, IGroupHandle
    {
        ...
        protected override void Unbind()
        {
            if (!IgnoreUnbind)
            {
                UnsafeNativeMethods.group_delete(this);
        ...
        internal TableHandle GetTable(String name)
        {
            ...
            {              
                th.SetHandle(UnsafeNativeMethods.group_get_or_add_table(this, name, (IntPtr)name.Length));//if something goes wrong in c++ land IntPtr.Zero is returned
        ...
        internal TableHandle GetTable(long tableIndex)
        {
            ...
            {
                th.SetHandle(UnsafeNativeMethods.group_get_table_by_index(this, tableIndex));
            }//at this point we have atomically acquired a handle and also set the root correctly so it can be unbound correctly
            return th;
        }        
