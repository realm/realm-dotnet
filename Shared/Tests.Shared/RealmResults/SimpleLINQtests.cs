...
namespace IntegrationTests
{
    ...
    internal class SimpleLINQtests : PeopleTestsBase
    {
        ...
        {
            var s0 = _realm.GetAll<Person>().Where(p => p.Score == 42.42f).ToList();
            Assert.That(s0.Count(), Is.EqualTo(1));
            Assert.That(s0[0].Score, Is.EqualTo(42.42f));

            var s1 = _realm.GetAll<Person>().Where(p => p.Longitude < -70.0 && p.Longitude > -90.0).ToList();
            Assert.That(s1[0].Email, Is.EqualTo("john@doe.com"));

            var s2 = _realm.GetAll<Person>().Where(p => p.Longitude < 0).ToList();
        ...
        {
            var countSimpleNot = _realm.GetAll<Person>().Where(p => !p.IsInteresting).Count();
            Assert.That(countSimpleNot, Is.EqualTo(1));

            var countSimpleNot2 = _realm.GetAll<Person>().Count(p => !p.IsInteresting);
            Assert.That(countSimpleNot2, Is.EqualTo(1));

            var countNotEqual = _realm.GetAll<Person>().Where(p => !(p.Score == 42.42f)).Count();
            Assert.That(countNotEqual, Is.EqualTo(2));

            var countNotComplex = _realm.GetAll<Person>().Where(p => !(p.Longitude < -70.0 && p.Longitude > -90.0)).Count();
        ...
        {
            var r0 = _realm.GetAll<Person>().Where(p => p.Score == 42.42f);
            var c0 = r0.Count();  // defer so can check in debugger if RealmResults.Count() evaluated correctly
            Assert.That(c0, Is.EqualTo(1));

            var c1 = _realm.GetAll<Person>().Where(p => p.Latitude <= 50).Count();
            Assert.That(c1, Is.EqualTo(2));

            var c2 = _realm.GetAll<Person>().Where(p => p.IsInteresting).Count();
            Assert.That(c2, Is.EqualTo(2));

            var c3 = _realm.GetAll<Person>().Where(p => p.FirstName == "John").Count();
            Assert.That(c3, Is.EqualTo(2));

            var c4 = _realm.GetAll<Person>().Count(p => p.FirstName == "John");
        ...
        {
            var c0 = _realm.GetAll<Person>().Where(p => p.Score == 3.14159f).Count();
            Assert.That(c0, Is.EqualTo(0));

            var c1 = _realm.GetAll<Person>().Where(p => p.Latitude > 88).Count();
            Assert.That(c1, Is.EqualTo(0));

            var c3 = _realm.GetAll<Person>().Where(p => p.FirstName == "Samantha").Count();
        ...
        {
            var s0 = _realm.GetAll<Person>().Where(p => p.Latitude == 40.7637286);
            Assert.That(s0.Count, Is.EqualTo(1));
            Assert.That(s0.ToList()[0].Latitude, Is.EqualTo(40.7637286));

            var s1 = _realm.GetAll<Person>().Where(p => p.Latitude != 40.7637286).ToList();
            Assert.That(s1.Count, Is.EqualTo(2));
            Assert.That(s1[0].Latitude, Is.EqualTo(51.508530));
            Assert.That(s1[1].Latitude, Is.EqualTo(37.7798657));

            var s2 = _realm.GetAll<Person>().Where(p => p.Latitude < 40).ToList();
            Assert.That(s2.Count, Is.EqualTo(1));
            Assert.That(s2[0].Latitude, Is.EqualTo(37.7798657));

            var s3 = _realm.GetAll<Person>().Where(p => p.Latitude <= 40.7637286).ToList();
            Assert.That(s3.Count, Is.EqualTo(2));
            Assert.That(s3[0].Latitude, Is.EqualTo(40.7637286));
            Assert.That(s3[1].Latitude, Is.EqualTo(37.7798657));

            var s4 = _realm.GetAll<Person>().Where(p => p.Latitude > 50).ToList();
            Assert.That(s4.Count, Is.EqualTo(1));
            Assert.That(s4[0].Latitude, Is.EqualTo(51.508530));

            var s5 = _realm.GetAll<Person>().Where(p => p.Latitude >= 51.508530).ToList();
            Assert.That(s5.Count, Is.EqualTo(1));
        ...
        public void SearchComparingLong()
        {
            ...
            var between = _realm.GetAll<Person>().Where(p => p.Salary > 30000 && p.Salary < 87000).ToArray();
        ...
        {
            var equality = _realm.GetAll<Person>().Where(p => p.LastName == "Smith").ToArray();
            Assert.That(equality.Length, Is.EqualTo(1));
            Assert.That(equality[0].FullName, Is.EqualTo("John Smith"));

            var contains = _realm.GetAll<Person>().Where(p => p.FirstName.Contains("et")).ToArray();
            Assert.That(contains.Length, Is.EqualTo(1));
            Assert.That(contains[0].FullName, Is.EqualTo("Peter Jameson"));

            var startsWith = _realm.GetAll<Person>().Where(p => p.Email.StartsWith("john@")).ToArray();
            Assert.That(startsWith.Length, Is.EqualTo(2));
            Assert.That(startsWith.All(p => p.FirstName == "John"), Is.True);

            var endsWith = _realm.GetAll<Person>().Where(p => p.Email.EndsWith(".net")).ToArray();
            Assert.That(endsWith.Length, Is.EqualTo(1));
            Assert.That(endsWith[0].FullName, Is.EqualTo("Peter Jameson"));

            var @null = _realm.GetAll<Person>().Where(p => p.OptionalAddress == null).ToArray();
            ...
            var null_or_empty = _realm.GetAll<Person>().Where(p => string.IsNullOrEmpty(p.OptionalAddress));
        ...
        public void SearchComparingDateTimeOffset()
        {
            ...
            var equality = _realm.GetAll<Person>().Where(p => p.Birthday == bdayPeterJameson).ToArray();
            Assert.That(equality.Length, Is.EqualTo(1));
            Assert.That(equality[0].FullName, Is.EqualTo("Peter Jameson"));

            var lessThan = _realm.GetAll<Person>().Where(p => p.Birthday < d1960).ToArray();
            Assert.That(lessThan.Length, Is.EqualTo(1));
            Assert.That(lessThan[0].FullName, Is.EqualTo("John Smith"));

            var lessOrEqualThan = _realm.GetAll<Person>().Where(p => p.Birthday <= bdayJohnDoe).ToArray();
            Assert.That(lessOrEqualThan.Length, Is.EqualTo(2));
            Assert.That(lessOrEqualThan.All(p => p.FirstName == "John"), Is.True);

            var greaterThan = _realm.GetAll<Person>().Where(p => p.Birthday > d1970).ToArray();
            Assert.That(greaterThan.Length, Is.EqualTo(1));
            Assert.That(greaterThan[0].FullName, Is.EqualTo("Peter Jameson"));

            var greaterOrEqualThan = _realm.GetAll<Person>().Where(p => p.Birthday >= bdayJohnDoe).ToArray();
            Assert.That(greaterOrEqualThan.Length, Is.EqualTo(2));
            Assert.That(greaterOrEqualThan.Any(p => p.FullName == "John Doe") && greaterOrEqualThan.Any(p => p.FullName == "Peter Jameson"), Is.True);

            var between = _realm.GetAll<Person>().Where(p => p.Birthday > d1960 && p.Birthday < d1970).ToArray();
            Assert.That(between.Length, Is.EqualTo(1));
            Assert.That(between[0].FullName, Is.EqualTo("John Doe"));
        }
        ...
        {
            var @null = _realm.GetAll<Person>().Where(p => p.IsAmbivalent == null);
            Assert.That(@null.Single().FullName, Is.EqualTo("Peter Jameson"));

            var not_null = _realm.GetAll<Person>().Where(p => p.IsAmbivalent != null);
            Assert.That(not_null.Count(), Is.EqualTo(2));

            var @true = _realm.GetAll<Person>().Where(p => p.IsAmbivalent == true);
            Assert.That(@true.Single().FullName, Is.EqualTo("John Smith"));

            var @false = _realm.GetAll<Person>().Where(p => p.IsAmbivalent == false);
        ...
        public void SearchComparingByteArrays()
        {
            ...
            var equality = _realm.GetAll<Person>().Where(p => p.PublicCertificateBytes == cafebabe);
            Assert.That(equality.Single().PublicCertificateBytes, Is.EqualTo(cafebabe));

            var unequality = _realm.GetAll<Person>().Where(p => p.PublicCertificateBytes != deadbeef);
            Assert.That(unequality.Count(), Is.EqualTo(2));

            var emptyness = _realm.GetAll<Person>().Where(p => p.PublicCertificateBytes == empty);
            Assert.That(emptyness, Is.Empty);

            // we should support this as well - see #570
            // var @null = _realm.GetAll<Person>().Where(p => p.PublicCertificateBytes == null);
        ...
        public void SearchComparingChar()
        {
            ...
            var equality = _realm.GetAll<PrimaryKeyCharObject>().Where(p => p.CharProperty == 'A').ToArray();
            Assert.That(equality.Select(p => p.CharProperty), Is.EquivalentTo(new[] { 'A' }));

            var inequality = _realm.GetAll<PrimaryKeyCharObject>().Where(p => p.CharProperty != 'c').ToArray();
            Assert.That(inequality.Select(p => p.CharProperty), Is.EquivalentTo(new[] { 'A', 'B', 'a' }));

            var lessThan = _realm.GetAll<PrimaryKeyCharObject>().Where(p => p.CharProperty < 'c').ToArray();
            Assert.That(lessThan.Select(p => p.CharProperty), Is.EquivalentTo(new[] { 'A', 'B', 'a' }));

            var lessOrEqualThan = _realm.GetAll<PrimaryKeyCharObject>().Where(p => p.CharProperty <= 'c').ToArray();
            Assert.That(lessOrEqualThan.Select(p => p.CharProperty), Is.EquivalentTo(new[] { 'A', 'B', 'a', 'c' }));

            var greaterThan = _realm.GetAll<PrimaryKeyCharObject>().Where(p => p.CharProperty > 'a').ToArray();
            Assert.That(greaterThan.Select(p => p.CharProperty), Is.EquivalentTo(new[] { 'c' }));

            var greaterOrEqualThan = _realm.GetAll<PrimaryKeyCharObject>().Where(p => p.CharProperty >= 'B').ToArray();
        ...
        {
            Assert.That(_realm.GetAll<Person>().Where(p => p.Latitude > 50).Any());
            Assert.That(_realm.GetAll<Person>().Where(p => p.Score > 0).Any());
            Assert.That(_realm.GetAll<Person>().Where(p => p.IsInteresting == false).Any());
            Assert.That(_realm.GetAll<Person>().Where(p => p.FirstName == "John").Any());
        }

        [Test]
        public void AnyFails()
        {
            Assert.False(_realm.GetAll<Person>().Where(p => p.Latitude > 100).Any());
            Assert.False(_realm.GetAll<Person>().Where(p => p.Score > 50000).Any());
            Assert.False(_realm.GetAll<Person>().Where(p => p.FirstName == "Samantha").Any());
        }

        [Test]
        public void SingleFailsToFind()
        {
            Assert.Throws<InvalidOperationException>(() => _realm.GetAll<Person>().Single(p => p.Latitude > 100));
            Assert.Throws<InvalidOperationException>(() => _realm.GetAll<Person>().Where(p => p.Latitude > 100).Single());
            Assert.Throws<InvalidOperationException>(() => _realm.GetAll<Person>().Single(p => p.Score > 50000));
            Assert.Throws<InvalidOperationException>(() => _realm.GetAll<Person>().Single(p => p.FirstName == "Samantha"));
        }

        [Test]
        public void SingleFindsTooMany()
        {
            Assert.Throws<InvalidOperationException>(() => _realm.GetAll<Person>().Where(p => p.Latitude == 50).Single());
            Assert.Throws<InvalidOperationException>(() => _realm.GetAll<Person>().Single(p => p.Score != 100.0f));
            Assert.Throws<InvalidOperationException>(() => _realm.GetAll<Person>().Single(p => p.FirstName == "John"));
        }

        [Test]
        public void SingleWorks()
        {
            var s0 = _realm.GetAll<Person>().Single(p => p.Longitude < -70.0 && p.Longitude > -90.0);
            Assert.That(s0.Email, Is.EqualTo("john@doe.com"));

            var s1 = _realm.GetAll<Person>().Where(p => p.Score == 100.0f).Single();
            Assert.That(s1.Email, Is.EqualTo("john@doe.com"));

            var s2 = _realm.GetAll<Person>().Single(p => p.FirstName == "Peter");
        ...
        {
            var s0 = _realm.GetAll<Person>().SingleOrDefault(p => p.Longitude < -70.0 && p.Longitude > -90.0);
            Assert.That(s0.Email, Is.EqualTo("john@doe.com"));

            var s1 = _realm.GetAll<Person>().Where(p => p.Score == 100.0f).SingleOrDefault();
            Assert.That(s1.Email, Is.EqualTo("john@doe.com"));

            var s2 = _realm.GetAll<Person>().SingleOrDefault(p => p.FirstName == "Peter");
            Assert.That(s2.FirstName, Is.EqualTo("Peter"));
        }
        ...
        {
            var expectedDef = _realm.GetAll<Person>().SingleOrDefault(p => p.FirstName == "Zaphod");
            Assert.That(expectedDef, Is.Null);

            expectedDef = _realm.GetAll<Person>().Where(p => p.FirstName == "Just Some Guy").SingleOrDefault();
            Assert.That(expectedDef, Is.Null);
        }
        ...
        {
            Assert.Throws<InvalidOperationException>(() => _realm.GetAll<Person>().First(p => p.Latitude > 100));
            Assert.Throws<InvalidOperationException>(() => _realm.GetAll<Person>().Where(p => p.Latitude > 100).First());
            Assert.Throws<InvalidOperationException>(() => _realm.GetAll<Person>().First(p => p.FirstName == "Samantha"));
        }

        [Test]
        public void FirstWorks()
        {
            var s0 = _realm.GetAll<Person>().First(p => p.Longitude < -70.0 && p.Longitude > -90.0);
            Assert.That(s0.Email, Is.EqualTo("john@doe.com"));

            var s1 = _realm.GetAll<Person>().Where(p => p.Score == 100.0f).First();
            Assert.That(s1.Email, Is.EqualTo("john@doe.com"));

            var s2 = _realm.GetAll<Person>().First(p => p.FirstName == "John");
        ...
        {
            var s0 = _realm.GetAll<Person>().FirstOrDefault(p => p.Longitude < -70.0 && p.Longitude > -90.0);
            Assert.That(s0.Email, Is.EqualTo("john@doe.com"));

            var s1 = _realm.GetAll<Person>().Where(p => p.Score == 100.0f).FirstOrDefault();
            Assert.That(s1.Email, Is.EqualTo("john@doe.com"));

            var s2 = _realm.GetAll<Person>().FirstOrDefault(p => p.FirstName == "John");
            Assert.That(s2.FirstName, Is.EqualTo("John"));
        }
        ...
        {
            var expectedDef = _realm.GetAll<Person>().FirstOrDefault(p => p.FirstName == "Zaphod");
            Assert.That(expectedDef, Is.Null);

            expectedDef = _realm.GetAll<Person>().Where(p => p.FirstName == "Just Some Guy").FirstOrDefault();
            Assert.That(expectedDef, Is.Null);
        }
        ...
        {
            Assert.Throws<InvalidOperationException>(() => _realm.GetAll<Person>().Last(p => p.Latitude > 100));
            Assert.Throws<InvalidOperationException>(() => _realm.GetAll<Person>().Where(p => p.Latitude > 100).Last());
            Assert.Throws<InvalidOperationException>(() => _realm.GetAll<Person>().Last(p => p.LastName == "Samantha"));
        }

        [Test]
        public void LastWorks()
        {
            var s0 = _realm.GetAll<Person>().Last(p => p.Longitude < -70.0 && p.Longitude > -90.0);
            Assert.That(s0.Email, Is.EqualTo("john@doe.com"));  // Last same as First when one match

            var s1 = _realm.GetAll<Person>().Where(p => p.Score == 100.0f).Last();
            Assert.That(s1.Email, Is.EqualTo("john@doe.com"));

            var s2 = _realm.GetAll<Person>().Last(p => p.FirstName == "John");
        ...
        {
            var s0 = _realm.GetAll<Person>().LastOrDefault(p => p.Longitude < -70.0 && p.Longitude > -90.0);
            Assert.That(s0.Email, Is.EqualTo("john@doe.com"));  // Last same as First when one match

            var s1 = _realm.GetAll<Person>().Where(p => p.Score == 100.0f).LastOrDefault();
            Assert.That(s1.Email, Is.EqualTo("john@doe.com"));

            var s2 = _realm.GetAll<Person>().LastOrDefault(p => p.FirstName == "John");
            Assert.That(s2.FirstName, Is.EqualTo("John"));  // order not guaranteed in two items but know they match this
        }
        ...
        {
            var expectedDef = _realm.GetAll<Person>().LastOrDefault(p => p.FirstName == "Zaphod");
            Assert.That(expectedDef, Is.Null);

            expectedDef = _realm.GetAll<Person>().Where(p => p.FirstName == "Just Some Guy").LastOrDefault();
            Assert.That(expectedDef, Is.Null);
        }
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => _realm.GetAll<Person>().Where(p => p.Latitude > 140).ElementAt(0));
            Assert.Throws<ArgumentOutOfRangeException>(() => _realm.GetAll<Person>().Where(p => p.FirstName == "Samantha").ElementAt(0));
            Assert.Throws<ArgumentOutOfRangeException>(() => _realm.GetAll<Person>().Where(p => p.FirstName == "Peter").ElementAt(10));
            Assert.Throws<ArgumentOutOfRangeException>(() => _realm.GetAll<Person>().ElementAt(10));
            Assert.Throws<ArgumentOutOfRangeException>(() => _realm.GetAll<Person>().ElementAt(-1));
        }

        [Test]
        public void ElementAtInRange()
        {
            var s0 = _realm.GetAll<Person>().Where(p => p.Longitude < -70.0 && p.Longitude > -90.0).ElementAt(0);
            Assert.That(s0.Email, Is.EqualTo("john@doe.com"));

            var s1 = _realm.GetAll<Person>().Where(p => p.Score == 100.0f).ElementAt(0);
            Assert.That(s1.Email, Is.EqualTo("john@doe.com"));

            var s2 = _realm.GetAll<Person>().Where(p => p.FirstName == "John").ElementAt(1);
            Assert.That(s2.FirstName, Is.EqualTo("John"));

            var s3 = _realm.GetAll<Person>().ElementAt(2);
        ...
        {
            var s0 = _realm.GetAll<Person>().Where(p => p.Longitude < -70.0 && p.Longitude > -90.0).ElementAtOrDefault(0);
            Assert.That(s0.Email, Is.EqualTo("john@doe.com"));

            var s1 = _realm.GetAll<Person>().Where(p => p.Score == 100.0f).ElementAtOrDefault(0);
            Assert.That(s1.Email, Is.EqualTo("john@doe.com"));

            var s2 = _realm.GetAll<Person>().Where(p => p.FirstName == "John").ElementAtOrDefault(1);
            Assert.That(s2.FirstName, Is.EqualTo("John"));

            var s3 = _realm.GetAll<Person>().ElementAtOrDefault(2);
            Assert.That(s3.FirstName, Is.EqualTo("Peter"));
        }
        ...
        {
            var expectedDef = _realm.GetAll<Person>().Where(p => p.FirstName == "Just Some Guy").ElementAtOrDefault(0);
            Assert.That(expectedDef, Is.Null);
        }
        ...
        public void DefaultIfEmptyReturnsDefault()
        {
             ...
             */
            var expectCollectionOfOne = _realm.GetAll<Person>().Where(p => p.FirstName == "Just Some Guy").DefaultIfEmpty();
