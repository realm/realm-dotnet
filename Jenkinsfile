...
stage('Checkout') {
  rlmNode('docker') {
    ...
    else if (env.CHANGE_BRANCH == 'release/10.0.0-beta.7') {
      versionSuffix = "beta.7"
    }
...
stage('Test') {
  ...
  def jobs = [
    ...
    '.NET Core macOS': NetCoreTest('macos && net5', 'netcoreapp3.1'),
    ...
    rlmNode(nodeName) {
      ...
      if (isUnix()) {
        if (nodeName == 'docker') {
          ...
          if (targetFramework.equals("netcoreapp3.1")) {
            ...
            test_runner_image = docker.build("netcoreapp3.1_custom", "./dockerfile_custom_netcore31")
          } else {
            test_runner_image = docker.image(DetermineDockerImg(targetFramework))
