...
class CommonUtils
{
    ...
    async VerifyConsistentHashing()
    {
        ...
        hash = await utils.tryGetHash([twoUp], this.oss);
        if (hash !== undefined)
        {
            hashMap.set(hash, twoUp);
        }
        else
        {
            assert.fail(`It was impossible to calculate the hash for ${twoUp}`);
        }
        ...
        if (hash !== undefined)
        {
            assert.equal(hashMap.get(hash), twoUp);
        }
        else
        {
            assert.fail(`It was impossible to re-calculate the hash for ${twoUp}`);
        }
    }
}