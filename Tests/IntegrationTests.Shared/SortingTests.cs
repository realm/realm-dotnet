...
namespace IntegrationTests
{
    class SortingTests : PeopleTestsBase
    {
        ...
        public void AllSortOneLevel()
        {
            ...
            Assert.That(s0, Is.EqualTo( new [] {-0.9907f, 42.42f, 100.0f}) );
        ...
        public void AllSortTwoLevel()
        {
            ...
            Assert.That(sortAA, Is.EqualTo( new [] {40.7637286, 51.508530, 37.7798657}) );
            ...
            Assert.That(sortAD, Is.EqualTo( new [] {51.508530, 40.7637286, 37.7798657}) );

            var sortDD = _realm.All<Person>().OrderByDescending(p => p.FirstName).ThenByDescending(p => p.Latitude).ToList().Select(p => p.Latitude);
            Assert.That(sortDD, Is.EqualTo( new [] {37.7798657, 51.508530, 40.7637286}) );
        }
