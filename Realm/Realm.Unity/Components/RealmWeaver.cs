...
namespace Weaver
{
    public class RealmWeaver : WeaverBase
    {
        ...
        private ImportedReferences _references;

        public RealmWeaver(ModuleDefinition module, ILogger log) : base(module, log)
        {
        }

        private IEnumerable<TypeDefinition> GetMatchingTypes()
        {
            foreach (var type in ModuleDefinition.Types.Where(t => t.IsRealmObjectInheritor(_references) || t.IsEmbeddedObjectInheritor(_references)))
            {
                ...
                {
                    Logger.Error($"The type {type.FullName} indirectly inherits from RealmObject which is not supported.", type.GetConstructors().FirstOrDefault()?.GetLocation());
                }
            }
        }

        protected override WeaveModuleResult Execute()
        {
            _references = ImportedReferences.Create(ModuleDefinition, new FrameworkName(".NETFramework,Version=v4.7.2"));

            if (_references.WovenAssemblyAttribute == null)
            {
                return WeaveModuleResult.Skipped($"Not weaving assembly '{ModuleDefinition.Name}' because it doesn't reference Realm.");
            }

            var isWoven = ModuleDefinition.Assembly.CustomAttributes.Any(a => a.AttributeType.IsSameAs(_references.WovenAssemblyAttribute));
            if (isWoven)
            {
                return WeaveModuleResult.Skipped($"Not weaving assembly '{ModuleDefinition.Name}' because it has already been processed.");
            }

            var methodTable = new Dictionary<string, Accessors>();

            var matchingTypes = GetMatchingTypes().ToArray();
            var results = new List<WeaveTypeResult>();
            foreach (var type in matchingTypes)
            {
                try
                {
                    var weaveResult = WeaveType(type, methodTable);
                    if (weaveResult != null)
                    {
                        // Can be null if an error occurred, but we will log that separately.
                        results.Add(weaveResult);
                    }
                }
                catch (Exception e)
                {
                    Logger.Error($"Unexpected error caught weaving type '{type.Name}': {e.Message}.\r\nCallstack:\r\n{e.StackTrace}", type.GetLocation());
                }
            }

            return WeaveModuleResult.Success(results);
        }
        ...
        private WeaveTypeResult WeaveType(TypeDefinition type, Dictionary<string, Accessors> methodTable)
        {
            ...
            foreach (var prop in type.Properties.Where(x => x.HasThis && !x.CustomAttributes.Any(a => a.AttributeType.Name == "IgnoredAttribute")))
            {
                try
                {
                    ...
                    {
                        var location = prop.GetMethod.GetLocation();
                        if (!string.IsNullOrEmpty(weaveResult.ErrorMessage))
                        {
                            // We only want one error point, so even though there may be more problems, we only Logger.Error the first one.
                            Logger.Error(weaveResult.ErrorMessage, location);
                        }
                        else
                        {
                            if (!string.IsNullOrEmpty(weaveResult.WarningMessage))
                            {
                                Logger.Warning(weaveResult.WarningMessage, location);
                            }
                            ...
                            {
                                Logger.Error($"{type.Name}.{prop.Name} has {string.Join(", ", realmAttributeNames)} applied, but it's not persisted, so those attributes will be ignored.", location);
                            }
                        }
                    }
                }
                catch (Exception e)
                {
                    Logger.Error(
                        ...
                        prop.GetMethod.GetLocation());
                }
            }

            if (!persistedProperties.Any())
            {
                Logger.Error($"Class {type.Name} is a RealmObject but has no persisted properties.", type.GetLocation());
                return null;
            }
            ...
            {
                Logger.Error($"Class {type.Name} is an EmbeddedObject but has a primary key {pkProperty.Property.Name} defined.", pkProperty.Property.GetMethod.GetLocation());
                return null;
            }

            if (persistedProperties.Count(p => p.IsPrimaryKey) > 1)
            {
                Logger.Error($"Class {type.Name} has more than one property marked with [PrimaryKey].", type.GetLocation());
                return null;
            }

            var objectConstructor = type.GetConstructors().SingleOrDefault(c => c.Parameters.Count == 0 && !c.IsStatic);
            if (objectConstructor == null)
            {
                var nonDefaultConstructor = type.GetConstructors().First();
                Logger.Error($"Class {type.Name} must have a public constructor that takes no parameters.", nonDefaultConstructor.GetLocation());
                return null;
            }
            ...
            Logger.Debug($"Added [Preserve] to {type.Name} and its constructor.");
        ...
        private WeavePropertyResult WeaveProperty(PropertyDefinition prop, TypeDefinition type, Dictionary<string, Accessors> methodTable)
        {
            ...
            Logger.Debug($"Woven {type.Name}.{prop.Name} as a {prop.PropertyType.FullName}{primaryKeyMsg}{indexedMsg}.");
                ...
                */
                ...
                foreach (var prop in properties.Where(p => !p.IsPrimaryKey && !p.Property.IsCollection(out _)))
                {
                    ...
                    else
                    {
                        ...
                        Logger.Error($"{realmObjectType.Name}.{property.Name} does not have a setter and is not an IList. This is an error in Realm, so please file a bug report.", property.GetMethod.GetLocation());
                    }
                }
