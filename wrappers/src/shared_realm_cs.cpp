...
REALM_EXPORT void shared_realm_write_copy(const SharedRealm& realm, Configuration configuration, SyncConfiguration* sync_configuration, uint8_t* encryption_key, NativeException::Marshallable& ex)
{
    handle_errors(ex, [&]() {
        Realm::Config config;

        if (sync_configuration && sync_configuration->user) {
            config = get_shared_realm_config(configuration, *sync_configuration, nullptr, 0, nullptr, encryption_key);
        }
        else {
            config.path = Utf16StringAccessor(configuration.path, configuration.path_len);
            if (encryption_key) {
