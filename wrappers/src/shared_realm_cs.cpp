...
Realm::Config get_shared_realm_config(Configuration configuration, SyncConfiguration sync_configuration, SchemaObject* objects, int objects_length, SchemaProperty* properties, uint8_t* encryption_key)
{
    ...
    config.sync_config->error_handler = [managed_sync_configuration_base_handle = sync_configuration.managed_sync_configuration_base_handle](SharedSyncSession session, SyncError error) {
        ...
        s_session_error_callback(new SharedSyncSession(session), error.error_code.value(), to_capi_value(error.message), user_info_pairs.data(), user_info_pairs.size(), error.is_client_reset_requested(), managed_sync_configuration_base_handle);
    };
