...
if (-Not $Incremental) {
    Remove-Item $build_directory/* -Recurse -Force -ErrorAction Ignore > $null
    cmake -S "$PSScriptRoot" -B "$build_directory" -DCMAKE_BUILD_TYPE=$Configuration -GXcode `
...
xcodebuild -project "$build_directory/RealmDotnet.xcodeproj" -scheme realm-wrappers -configuration $Configuration @destinations
exit $LASTEXITCODE
