...
* Added `ProgressEstimate` to `SyncProgress`. `ProgressEstimate` is a float value between 0.0 and 1.0 that expresses the percentage estimate of the current progress. At the same time `SyncProgress.TransferredBytes` and `SyncProgress.TransferableBytes` have been marked as obsolete in favour of `SyncProgress.ProgressEstimate`.  (Issue [#3478](https://github.com/realm/realm-dotnet/issues/3478]))
